function [Xk, Uk, phi, status, matrices] = linearmpc(xref,uref,model, constraint, penalty, terminal, matrices)
...
matrices.lb(1:numx) = model.x0 - xref;
matrices.ub(1:numx) = model.x0 - xref; % Fix x0 so that it optimizes u only and x is changed by system dynamics
...
Xk = reshape(X(xinds), numx, N + 1) + xref;
Uk = reshape(X(uinds), numu, N) + uref;
...
littleAlt2 = sparse([zeros(size(G)), zeros(size(H))]);
