...
dim.ny = size(LTI.C, 1);        % Number of outputs
dim.nd=1;                           % Number of disturbance
dim.N=N;                            %horizon
...
aug_sys.x0 = [LTI.x0; disturbance];

aug_sys.yref = [LTI.yref];

obs_poles = [0.1*pole(sysd_lqr); 0.1];
...
S_N = predmod.S(1:end-dim.nx,:);
T_N = predmod.T(1:end-dim.nx,:);

predmode=predmodgen(aug_sys, dime);  
[He,he]=costgen(predmode,weighte,dime);

% Receding horizon implementation
xe=zeros(dime.nx,size(T, 2));
y=zeros(dime.ny,size(T, 2));
u_rec=zeros(dime.nu,size(T, 2));
xehat=zeros(dime.nx,size(T, 2));

xe(:,1)=aug_sys.x0;
xehat(:,1)=[0; 0; 0; 0; 0];
y(:,1)=aug_sys.C*aug_sys.x0;

Options = sdpsettings('verbose',0);

%L=L_obs;%place(aug_sys.A',aug_sys.C',[0.5; 0.4; 0.45;0.6;0.65])';      %observer gain
...
for k=1:size(T, 2)-1
    ...
    % Constraint = [];
    Constraint = [G_N * (T_N * xe_0(1:dim.nx) + S_N * (uostar - ur_N)) + (H_N * (uostar - ur_N)) <= psi_N;
                  Xf.A * ((predmod.T(end-dim.nx+1:end,:)* xe_0(1:dim.nx)) + (predmod.S(end-dim.nx+1:end,:)* (uostar - ur_N))) <= Xf.b];
    % Constraint = [(G_N * predmod.S(1:end-dim.nx, :) + H_N) * (uostar) <= (psi_N - (G_N * predmod.T(1:end-dim.nx, :) * xe_0(1:dim.nx)) + G_N * xr_N(1:end-dim.nx) + H_N*ur_N);
    %     Xf.A * predmod.S(end-dim.nx+1:end, :) * (uostar - ur_N) <= Xf.b - Xf.A * predmod.T(end-dim.nx+1:end, :) * xe_0(1:dim.nx) + Xf.A*xr];%define constraints
    ...
    FVAL(k) = 0.5*uostar'*He*uostar+(he*[xe_0; xre; ur])'*uostar;
    % Select the first input only
    u_rec(:,k)=uostar(1:dim.nu);

    % Compute the state/output evolution
    xe(:,k+1)=aug_sys.A*xe_0 + aug_sys.B*u_rec(:,k);
    tspan = [T(k), T(k+1)];
    xe(1:4,k+1) = NLSystemDynamics(xe_0(1:4), tspan, u_rec(:,k) + disturbance);
    y(:,k+1)=aug_sys.C*xe(:,k+1);
    clear u_uncon
        
    % Update extended-state estimation
    xehat(:,k+1)=aug_sys.A*xehat(:,k)+aug_sys.B*u_rec(:,k)+L_obs*(y(:,k)-aug_sys.C*xehat(:,k));
    
    
end

figure;
plot(xehat(end-dim.nd+1:end,:));
...
figure;
subplot(2, 2, 1);
plot(xe(1,:), 'LineWidth', 2), grid on;
subplot(2, 2, 2);
plot(xe(2,:), 'LineWidth', 2), grid on;
subplot(2, 2, 3);
plot(xe(3,:), 'LineWidth', 2), grid on;
subplot(2, 2, 4);
plot(xe(4,:), 'LineWidth', 2), grid on;
title("State Response");

figure;
plot(u_rec, 'LineWidth', 2), grid on;
title("Control Input");
