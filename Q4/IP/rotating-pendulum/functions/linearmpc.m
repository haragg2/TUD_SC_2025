...
matrices.ub(1:numx) = model.x0; % Fix x0 so that it optimizes u only and x is changed by system dynamics
...
H = matrices.H;
f = matrices.f' - ref'*matrices.H;  %define cost function
A = matrices.Alt;
b =  matrices.blt;
Aeq = matrices.Aeq;
beq = matrices.beq - dist;
[X, ~, status] = quadprog(H,f, A, b, Aeq, beq, matrices.lb, matrices.ub, [], Options);

% diagnostic = optimize(Constraint,H, Options);  %solve the problem
...
%               x_ >= matrices.lb];                  %define constraints
% 
% Objective = 0.5*(x_ - ref)'*matrices.H*(x_ - ref) + matrices.f'*(x_ - ref);  %define cost function
% 
